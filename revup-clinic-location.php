...
function revup_enqueue_maps_scripts()
{
  ...
  wp_register_script(
    'clinic-maps',
    plugin_dir_url(__FILE__) . 'public/js/revup-clinic-maps.js',
    array('jquery'),
    filemtime(plugin_dir_path(__FILE__) . 'public/js/revup-clinic-maps.js'),
    true
  );

  wp_enqueue_script('clinic-maps');

  $api_key = get_option('revup_google_maps_api_key', '');
  if (!empty($api_key)) {
    wp_enqueue_script(
      'google-maps-places',
      "https://maps.googleapis.com/maps/api/js?key={$api_key}&libraries=places&callback=revUpInitMaps",
      array('clinic-maps'),
      null,
      true
    );
